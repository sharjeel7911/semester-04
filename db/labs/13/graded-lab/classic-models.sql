...
FROM products p1
WHERE p1.MSRP > (SELECT AVG(p2.MSRP)
				 FROM products p2
...
SELECT o.orderNumber, o.customerNumber,
       (SELECT SUM(od.quantityOrdered * od.priceEach) 
        ...
        WHERE od.orderNumber = o.orderNumber) AS totalOrderValue
FROM orders o
...
-- Question - 08
SELECT DISTINCT m.employeeNumber, m.firstName, m.lastName, m.reportsTo
FROM employees m
...
SELECT c.*
FROM customers c
WHERE c.customerNumber IN (SELECT p1.customerNumber
						   FROM payments p1
                           WHERE p1.amount = (SELECT MAX(p2.amount) 
											  FROM payments p2)
);
...
-- Question - 10
SELECT productName, MSRP, productLine
FROM products
WHERE MSRP > ALL (
    SELECT MSRP 
    FROM products 
